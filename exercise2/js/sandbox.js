$(document).ready(function() {
  ...
  $('input[type="text"]').parent().addClass('myForm');
  ...
  var index = $('#myList li.current').removeClass('current').index();
  $('#myList li').eq(index + 1).addClass('current');

  //Select the select element inside #specials; traverse your way to the submit button.
  $('#specials select[name="day"]').parent().siblings().children($('input[type="submit"]'));
